...
class QuestLogViewModel: ObservableObject {
    ...
    func fetchAllQuests() {
        ...
        listener = db.collection("quests")
            ...
            .addSnapshotListener { snapshot, error in
                if let error = error {
                    print(
                        "Fehler beim Abrufen der Quests: \(error.localizedDescription)"
                    )
                    return
                }
    ...
    var filteredQuests: [Quest] {
        quests.filter { quest in
            let isInRange =
                quest.createdAt >= startDate && quest.createdAt <= endDate
    ...
    func addQuest(
        ...
        do {
            ...
            print(
                "Fehler beim Hinzufügen der Quest: \(error.localizedDescription)"
            )
        }
    }
    ...
    func updateQuest(_ quest: Quest) {
        ...
        do {
            ...
            print(
                "Fehler beim Aktualisieren der Quest: \(error.localizedDescription)"
            )
        }
    }
    ...
    func deleteQuest(_ quest: Quest) {
        ...
        db.collection("quests").document(questId).delete { error in
            if let error = error {
                print(
                    "Fehler beim Löschen der Quest: \(error.localizedDescription)"
                )
            }
        }
    }
    ...
    func assignCharactersToQuest(quest: Quest, characterIds: [String]) {
        ...
        do {
            ...
            print(
                "Fehler beim Zuweisen der Charaktere: \(error.localizedDescription)"
            )
        }
    }
}
